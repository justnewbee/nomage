...
function getUInit16(arrUInt8, offset) {
	return (0, _bit.bitOr)((0, _bit.bitShiftL)(arrUInt8[offset], 8), arrUInt8[offset + 1]);
}
/**
 * 
 * @param {Uint8Array} arrUInt8
...
function decodeScan(data, offset, frame, components, resetInterval, spectralStart, spectralEnd, successivePrev, successive) {
	...
	function readBit() {
		if (bitsCount > 0) {
			bitsCount--;
			return (0, _bit.bitAnd)((0, _bit.bitShiftR)(bitsData, bitsCount), 1);
		}
		bitsData = data[offset++];
		if (bitsData == 0xFF) {
			var nextByte = data[offset++];
			if (nextByte) {
				throw new Error("unexpected marker: " + (0, _bit.bitOr)((0, _bit.bitShiftL)(bitsData, 8), nextByte).toString(16));
			}
		}
		bitsCount = 7;
		return bitsData >>> 7;
	...
	function receive(length) {
		...
		while (length > 0) {
			...
			n = (0, _bit.bitOr)((0, _bit.bitShiftL)(n, 1), bit);
			length--;
		}
		return n;
	}
	...
	function receiveAndExtend(length) {
		...
		if (n >= (0, _bit.bitShiftL)(1, length - 1)) {
			return n;
		}
		return n + (0, _bit.bitShiftL)(-1, length) + 1;
	}
	...
	function decodeBaseline(component, zz) {
		...
		while (k < 64) {
			...
			var s = (0, _bit.bitAnd)(rs, 15);
			var r = (0, _bit.bitShiftR)(rs, 4);
	...
	function decodeDCFirst(component, zz) {
		var t = decodeHuffman(component.huffmanTableDC);
		var diff = t === 0 ? 0 : (0, _bit.bitShiftL)(receiveAndExtend(t), successive);

		zz[0] = component.pred += diff;
	}

	function decodeDCSuccessive(component, zz) {
		zz[0] |= (0, _bit.bitShiftL)(readBit(), successive);
	}
	...
	function decodeACFirst(component, zz) {
		...
		while (k <= e) {
			...
			if (s === 0) {
				if (r < 15) {
					eobrun = receive(r) + (0, _bit.bitShiftL)(1, r) - 1;
					break;
				}
				k += 16;
				continue;
			...
			zz[dctZigZag[k]] = receiveAndExtend(s) * (0, _bit.bitShiftL)(1, successive);
		...
		while (k <= e) {
			...
			switch (successiveACState) {
				case 0:
					...
					if (s === 0) {
						if (r < 15) {
							eobrun = receive(r) + (0, _bit.bitShiftL)(1, r);
							successiveACState = 4;
						} else {
							r = 16;
							successiveACState = 1;
				...
				case 2:
					if (zz[z]) {
						zz[z] += (0, _bit.bitShiftL)(readBit(), successive);
					} else {
						r--;
						if (r === 0) {
				...
				case 3:
					...
					if (zz[z]) {
						...
						zz[z] = (0, _bit.bitShiftL)(successiveACNextValue, successive);
						successiveACState = 0;
					}
					break;
				case 4:
	...
	function decodeMcu(component, decode, mcu, row, col) {
		var mcuRow = (0, _bit.bitOr)(mcu / mcusPerLine, 0);
	...
	function decodeBlock(component, decode, mcu) {
		var blockRow = (0, _bit.bitOr)(mcu / component.blocksPerLine, 0);
	...
	while (mcu < mcuExpected) {
		...
		// find marker
		bitsCount = 0;
		marker = (0, _bit.bitOr)((0, _bit.bitShiftL)(data[offset], 8), data[offset + 1]);
		if (marker < 0xFF00) {
...
function buildComponentData(component) {
	...
	var samplesPerLine = (0, _bit.bitShiftL)(blocksPerLine, 3);
	...
	function quantizeAndInverse(zz, dataOut, dataIn) {
		...
		for (i = 0; i < 8; ++i) {
			...
			if (p[row + 1] == 0 && p[row + 2] == 0 && p[row + 3] == 0 && p[row + 4] == 0 && p[row + 5] == 0 && p[row + 6] == 0 && p[row + 7] == 0) {
				t = (0, _bit.bitShiftR)(dctSqrt2 * p[row] + 512, 10);
				p[row] = t;
				p[row + 1] = t;
				p[row + 2] = t;
				p[row + 3] = t;
			...
			// stage 4
			v0 = (0, _bit.bitShiftR)(dctSqrt2 * p[row] + 128, 8);
			v1 = (0, _bit.bitShiftR)(dctSqrt2 * p[row + 4] + 128, 8);
			v2 = p[row + 2];
			v3 = p[row + 6];
			v4 = (0, _bit.bitShiftR)(dctSqrt1d2 * (p[row + 1] - p[row + 7]) + 128, 8);
			v7 = (0, _bit.bitShiftR)(dctSqrt1d2 * (p[row + 1] + p[row + 7]) + 128, 8);
			v5 = (0, _bit.bitShiftL)(p[row + 3], 4);
			v6 = (0, _bit.bitShiftL)(p[row + 5], 4);

			// stage 3
			t = (0, _bit.bitShiftR)(v0 - v1 + 1, 1);
			v0 = (0, _bit.bitShiftR)(v0 + v1 + 1, 1);
			v1 = t;
			t = (0, _bit.bitShiftR)(v2 * dctSin6 + v3 * dctCos6 + 128, 8);
			v2 = (0, _bit.bitShiftR)(v2 * dctCos6 - v3 * dctSin6 + 128, 8);
			v3 = t;
			t = (0, _bit.bitShiftR)(v4 - v6 + 1, 1);
			v4 = (0, _bit.bitShiftR)(v4 + v6 + 1, 1);
			v6 = t;
			t = (0, _bit.bitShiftR)(v7 + v5 + 1, 1);
			v5 = (0, _bit.bitShiftR)(v7 - v5 + 1, 1);
			v7 = t;

			// stage 2
			t = (0, _bit.bitShiftR)(v0 - v3 + 1, 1);
			v0 = (0, _bit.bitShiftR)(v0 + v3 + 1, 1);
			v3 = t;
			t = (0, _bit.bitShiftR)(v1 - v2 + 1, 1);
			v1 = (0, _bit.bitShiftR)(v1 + v2 + 1, 1);
			v2 = t;
			t = (0, _bit.bitShiftR)(v4 * dctSin3 + v7 * dctCos3 + 2048, 12);
			v4 = (0, _bit.bitShiftR)(v4 * dctCos3 - v7 * dctSin3 + 2048, 12);
			v7 = t;
			t = (0, _bit.bitShiftR)(v5 * dctSin1 + v6 * dctCos1 + 2048, 12);
			v5 = (0, _bit.bitShiftR)(v5 * dctCos1 - v6 * dctSin1 + 2048, 12);
			v6 = t;
		...
		for (i = 0; i < 8; ++i) {
			...
			if (p[col + 8] == 0 && p[col + 8 * 2] == 0 && p[col + 8 * 3] == 0 && p[col + 8 * 4] == 0 && p[col + 8 * 5] == 0 && p[col + 8 * 6] == 0 && p[col + 8 * 7] == 0) {
				t = (0, _bit.bitShiftR)(dctSqrt2 * dataIn[i] + 8192, 14);
				p[col] = t;
				p[col + 8] = t;
				p[col + 8 * 2] = t;
				p[col + 8 * 3] = t;
			...
			// stage 4
			v0 = (0, _bit.bitShiftR)(dctSqrt2 * p[col] + 2048, 12);
			v1 = (0, _bit.bitShiftR)(dctSqrt2 * p[col + 8 * 4] + 2048, 12);
			v2 = p[col + 8 * 2];
			v3 = p[col + 8 * 6];
			v4 = (0, _bit.bitShiftR)(dctSqrt1d2 * (p[col + 8] - p[col + 8 * 7]) + 2048, 12);
			v7 = (0, _bit.bitShiftR)(dctSqrt1d2 * (p[col + 8] + p[col + 8 * 7]) + 2048, 12);
			v5 = p[col + 8 * 3];
			v6 = p[col + 8 * 5];
			...
			v0 = (0, _bit.bitShiftR)(v0 + v1 + 1, 1);
			v1 = t;
			t = (0, _bit.bitShiftR)(v2 * dctSin6 + v3 * dctCos6 + 2048, 12);
			v2 = (0, _bit.bitShiftR)(v2 * dctCos6 - v3 * dctSin6 + 2048, 12);
			v3 = t;
			t = (0, _bit.bitShiftR)(v4 - v6 + 1, 1);
			v4 = (0, _bit.bitShiftR)(v4 + v6 + 1, 1);
			v6 = t;
		...
		for (i = 0; i < 64; ++i) {
			var sample = 128 + (0, _bit.bitShiftR)(p[i] + 8, 4);
			dataOut[i] = sample < 0 ? 0 : sample > 0xFF ? 0xFF : sample;
		}
	}

	for (var blockRow = 0; blockRow < blocksPerColumn; blockRow++) {
		var scanLine = (0, _bit.bitShiftL)(blockRow, 3);
		for (i = 0; i < 8; i++) {
		...
		for (var blockCol = 0; blockCol < blocksPerLine; blockCol++) {
			...
			var sample = (0, _bit.bitShiftL)(blockCol, 3);
			var offset = 0;
...
function prepareComponents(frame) {
	...
	(0, _bit.each)(frame.components, function (v) {
		if (maxH < v.h) {
			maxH = v.h;
		}
		if (maxV < v.v) {
...
function parseAppJFIF(arrAppData) {
	if (arrAppData[0] === 0x4A && arrAppData[1] === 0x46 && arrAppData[2] === 0x49 && arrAppData[3] === 0x46 && arrAppData[4] === 0) {
		...
		return {
			...
			xDensity: (0, _bit.bitOr)((0, _bit.bitShiftL)(arrAppData[8], 8), arrAppData[9]),
			yDensity: (0, _bit.bitOr)((0, _bit.bitShiftL)(arrAppData[10], 8), arrAppData[11]),
...
function parseAppADOBE(arrAppData) {
	if (arrAppData[0] === 0x41 && arrAppData[1] === 0x64 && arrAppData[2] === 0x6F && arrAppData[3] === 0x62 && arrAppData[4] === 0x65 && arrAppData[5] === 0) {
		...
		return {
			...
			flags0: (0, _bit.bitOr)((0, _bit.bitShiftL)(arrAppData[7], 8), arrAppData[8]),
			flags1: (0, _bit.bitOr)((0, _bit.bitShiftL)(arrAppData[9], 8), arrAppData[10]),
...
function parseComponentData(buffer) {
		...
		switch (fileMarker) {
			...
			case _jpg2.default.DQT:
				...
				while (offset < quantizationTablesEnd) {
					...
					if ((0, _bit.bitShiftR)(quantizationTableSpec, 4) === 0) {
						// 8 bit values
						for (j = 0; j < 64; j++) {
							tableData[dctZigZag[j]] = data[offset++];
						}
					} else if ((0, _bit.bitShiftR)(quantizationTableSpec, 4) === 1) {
						// 16 bit
						for (j = 0; j < 64; j++) {
					...
					quantizationTables[(0, _bit.bitAnd)(quantizationTableSpec, 15)] = tableData;
				}
				break;
			...
			case _jpg2.default.SOF2:
				...
				for (i = 0; i < componentsCount; i++) {
					...
					var h = (0, _bit.bitShiftR)(data[offset + 1], 4);
					var v = (0, _bit.bitAnd)(data[offset + 1], 15);
			...
			case _jpg2.default.DHT:
				...
				for (i = 2; i < huffmanLength;) {
					...
					((0, _bit.bitShiftR)(huffmanTableSpec, 4) === 0 ? huffmanTablesDC : huffmanTablesAC)[(0, _bit.bitAnd)(huffmanTableSpec, 15)] = buildHuffmanTable(codeLengths, huffmanValues);
				}
				break;
			...
			case _jpg2.default.SOS:
				...
				for (i = 0; i < selectorsCount; i++) {
					...
					component.huffmanTableDC = huffmanTablesDC[(0, _bit.bitShiftR)(tableSpec, 4)];
					component.huffmanTableAC = huffmanTablesAC[(0, _bit.bitAnd)(tableSpec, 15)];
				...
				var processed = decodeScan(data, offset, frame, components, resetInterval, spectralStart, spectralEnd, (0, _bit.bitShiftR)(successiveApproximation, 4), (0, _bit.bitAnd)(successiveApproximation, 15));
	...
	(0, _bit.each)(frames[0].components, function (val) {
...
function getMiddleData(componentData) {
	...
	switch (components.length) {
		case 1:
			for (y = 0; y < height; y++) {
				component1Line = component1.lines[(0, _bit.bitOr)(0, y * component1.scaleY)];
				for (x = 0; x < width; x++) {
					Y = component1Line[(0, _bit.bitOr)(0, x * component1.scaleX)];
					data[offset++] = Y;
				}
			}
			break;
		case 2:
			...
			for (y = 0; y < height; y++) {
				component1Line = component1.lines[(0, _bit.bitOr)(0, y * component1.scaleY)];
				component2Line = component2.lines[(0, _bit.bitOr)(0, y * component2.scaleY)];
				for (x = 0; x < width; x++) {
					Y = component1Line[(0, _bit.bitOr)(0, x * component1.scaleX)];
					data[offset++] = Y;
					Y = component2Line[(0, _bit.bitOr)(0, x * component2.scaleX)];
					data[offset++] = Y;
				}
			}
			break;
		case 3:
			...
			for (y = 0; y < height; y++) {
				component1Line = component1.lines[(0, _bit.bitOr)(0, y * component1.scaleY)];
				component2Line = component2.lines[(0, _bit.bitOr)(0, y * component2.scaleY)];
				component3Line = component3.lines[(0, _bit.bitOr)(0, y * component3.scaleY)];
				for (x = 0; x < width; x++) {
					if (colorTransform) {
						Y = component1Line[(0, _bit.bitOr)(0, x * component1.scaleX)];
						Cb = component2Line[(0, _bit.bitOr)(0, x * component2.scaleX)];
						Cr = component3Line[(0, _bit.bitOr)(0, x * component3.scaleX)];

						R = clampTo8bit(Y + 1.402 * (Cr - 128));
						G = clampTo8bit(Y - 0.3441363 * (Cb - 128) - 0.71413636 * (Cr - 128));
						B = clampTo8bit(Y + 1.772 * (Cb - 128));
					} else {
						R = component1Line[(0, _bit.bitOr)(0, x * component1.scaleX)];
						G = component2Line[(0, _bit.bitOr)(0, x * component2.scaleX)];
						B = component3Line[(0, _bit.bitOr)(0, x * component3.scaleX)];
					}
		...
		case 4:
			...
			for (y = 0; y < height; y++) {
				...
				component2Line = component2.lines[(0, _bit.bitOr)(0, y * component2.scaleY)];
				component3Line = component3.lines[(0, _bit.bitOr)(0, y * component3.scaleY)];
				component4Line = component4.lines[(0, _bit.bitOr)(0, y * component4.scaleY)];
				for (x = 0; x < width; x++) {
					if (colorTransform) {
						Y = component1Line[(0, _bit.bitOr)(0, x * component1.scaleX)];
						Cb = component2Line[(0, _bit.bitOr)(0, x * component2.scaleX)];
						Cr = component3Line[(0, _bit.bitOr)(0, x * component3.scaleX)];
						K = component4Line[(0, _bit.bitOr)(0, x * component4.scaleX)];

						C = 255 - clampTo8bit(Y + 1.402 * (Cr - 128));
						M = 255 - clampTo8bit(Y - 0.3441363 * (Cb - 128) - 0.71413636 * (Cr - 128));
						Ye = 255 - clampTo8bit(Y + 1.772 * (Cb - 128));
					} else {
						C = component1Line[(0, _bit.bitOr)(0, x * component1.scaleX)];
						M = component2Line[(0, _bit.bitOr)(0, x * component2.scaleX)];
						Ye = component3Line[(0, _bit.bitOr)(0, x * component3.scaleX)];
						K = component4Line[(0, _bit.bitOr)(0, x * component4.scaleX)];
					}
					data[offset++] = 255 - C;
					data[offset++] = 255 - M;
